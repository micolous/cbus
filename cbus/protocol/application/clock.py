    ...
    def decode(cls, data, packet):
        ...
        while data:
            ...
            if (command_code & 0x80) == 0x80:
                warnings.warn(
                    'Got unknown clock command %02x; long form is not used.' %
                    command_code, UserWarning)
                break

            if (command_code & 0xE0) != 0:
                warnings.warn(
                    'Got unknown clock command %02x; don\'t know how to process the other bits.'
                    % command_code, UserWarning)
                break
            ...
            if (command_code & 0x08) == 0x08:
                ...
                # unknown
                warnings.warn(
                    'Got unknown clock command %02x; last stage dropout' %
                    command_code, UserWarning)
                break
...
class ClockUpdateSAL(ClockSAL):
    ...
    def decode(cls, data, packet, command_code):
        ...
        if variable == CLOCK_DATE:
            ...
            if data_length != 0x06:
                warnings.warn(
                    'Date variable being sent with length != 5 (got %d instead)'
                    % data_length, UserWarning)
                return None, data
            ...
            if data_length != 0x05:
                warnings.warn(
                    'Time variable being sent with length != 4 (got %d instead)'
                    % data_length, UserWarning)
                return None, data
            ...
            warnings.warn(
                'Tried to decode unknown clock update variable %02x' % variable,
                UserWarning)
    ...
    def encode(self):
        if self.variable == CLOCK_TIME:
            ...
            val = pack('>BBBB', self.val.hour, self.val.minute, self.val.second,
                       255)
        elif self.variable == CLOCK_DATE:
            # date
            val = pack('>HBBB', self.val.year, self.val.month, self.val.day,
                       self.val.weekday())
        else:
            # unknown
