    ...
    def decode(cls, data, packet):
        ...
        while data:
            ...
            if len(data) < 2:
                ...
                warnings.warn(
                    "Got 1 byte of stray SAL for lighting application (malformed packet)",
                    UserWarning)
                break
            ...
            if command_code not in SAL_HANDLERS:
                warnings.warn(
                    'Got unknown lighting command %r, stopping processing prematurely'
...
class LightingRampSAL(LightingSAL):
    ...
    def encode(self):
        ...
        if not validate_ramp_rate(self.duration):
            raise ValueError, 'Duration is out of bounds, must be between %d and %d (got %r)' % (
                MIN_RAMP_RATE, MAX_RAMP_RATE, self.duration)
    ...
    def __repr__(self):
        return '<LightingRampSAL object: group_address=%r, duration=%r, level=%r>' % (
            self.group_address, self.duration, self.level)
...
class LightingOnSAL(LightingSAL):
    ...
    def decode(cls, data, packet, command_code, group_address):
		...
		Do not call this method directly -- use LightingSAL.decode
		"""
        assert command_code == LIGHT_ON, "command_code (%r) != LIGHT_ON (%r)" % (
...
class LightingOffSAL(LightingSAL):
    ...
    def decode(cls, data, packet, command_code, group_address):
		...
		Do not call this method directly -- use LightingSAL.decode
		"""
        assert command_code == LIGHT_OFF, "command_code (%r) != LIGHT_OFF (%r)" % (
...
class LightingTerminateRampSAL(LightingSAL):
    ...
    def decode(cls, data, packet, command_code, group_address):
		...
		Do not call this method directly -- use LightingSAL.decode
		"""
        assert command_code == LIGHT_TERMINATE_RAMP, "command_code (%r) != LIGHT_TERMINATE_RAMP (%r)" % (
