	...
	def decode(cls, data, packet):
		...
		while data:
			...
			if len(data) < 3:
				...
				warnings.warn("Got less than 3 bytes of stray SAL for temperature application (malformed packet)", UserWarning)
				break
			...
			if (command_code & 0x80) == 0x80:
				warnings.warn('Got unknown temperature command %r, stopping processing prematurely' % command_code, UserWarning)
				break
			
			if (command_code & 0x07) != 2:
				warnings.warn('Got invalid length for temperature command %r, must be 2 (Temperature s9.4.1)' % command_code, UserWarning)
				break
				
			sal, data = TemperatureBroadcastSAL.decode(data, packet, command_code, group_address)
