...
def main():
    ...
    if options.serial_pci and options.tcp_pci:
        ...
        parser.error(
            'No CBus PCI listen address was specified!  (See -s or -t option)')
    ...
    reactor.callWhenRunning(boot, serial_mode, addr, options.daemon,
                            options.pid_file, options.session_bus)
    reactor.run()
